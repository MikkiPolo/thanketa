...
def mix_brand_items_v5(
    ...
    """
    # Если недостаточно капсул пользователя, дополняем брендовыми товарами
    if len(user_capsules) < 20:
        print(f"  🛍️ Недостаточно капсул пользователя ({len(user_capsules)}), дополняем до 20 брендовыми товарами")
        # Используем V4 для создания капсул из брендов, но с учетом вещей пользователя
        from brand_service_v4 import supplement_capsules_with_brand_items
        return supplement_capsules_with_brand_items(
            user_capsules=user_capsules,  # Передаем существующие капсулы
            target_count=20,
            season=season,
            temperature=temperature,
            user_wardrobe=wardrobe  # Передаем вещи пользователя
        )
