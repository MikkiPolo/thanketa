...
def translate_category(raw: str) -> str:
    """Локальная нормализация категорий для rule-engine. Возвращает одно из: tops,bottoms,dresses,outerwear,shoes,accessories,other"""
    if not raw:
        return "other"
    ...
    tops = {
        "топ", "топик", "кроптоп", "кроп-топ", "боди", "бра", "бралетт", "бралетт-топ", "корсет", "бюстье", "камисоль", "майка", "атлетика", "танк-топ", "футболка", "тишка", "лонгслив", "реглан", "хенли", "поло", "регби-рубаха", "рубашка", "оверсайз-рубашка", "оксфорд-рубашка", "блузка", "пеплум-топ", "баска-топ", "жилет-костюмный", "жилет-трикотажный", "свитшот", "свитер", "джемпер", "пуловер", "кардиган-тонкий", "водолазка", "гольф", "худи", "hoodie", "худи-платье", "худи-лонг", "кофта", "кофта на молнии", "олимпийка", "толстовка", "термодолазка", "топ-труба", "пижамная рубашка", "шёлковая рубашка", "шелковая рубашка", "топ на запах", "топ с запахом", "топ с одним плечом", "топ на бретелях", "топ-асимметрия", "кимоно-топ", "спортивный топ", "компрессионная футболка", "кроп-худи", "свитшот-кроп", "топ-майо", "топ-бардо", "топ-кармен"
    }
    
    # BOTTOMS — низ (брюки, джинсы, юбки, шорты и пр.)
    bottoms = {
        "брюки", "чинос", "чиносы", "слаксы", "палаццо", "карго-брюки", "карго", "джоггеры", "трико", "спортивные брюки", "штаны", "леггинсы", "легинсы", "treggings", "джинсы", "деним", "бойфренды", "мом-джинс", "скини", "слим", "прямые джинсы", "клёш", "клеш", "буткат", "flare", "bootcut", "карго-джинсы", "варёные джинсы", "варенки", "бананы", "paperbag", "каррот", "юбка", "мини-юбка", "миди-юбка", "макси-юбка", "юбка-карандаш", "плиссе", "плиссированная юбка", "солнце", "полусолнце", "годе", "саронг", "джинсовая юбка", "кожаная юбка", "шорты", "бермуды", "велосипедки", "капри", "бриджи", "кюлоты", "брюки-кюлоты", "брюки-дудочки", "брюки со стрелками", "брюки-карго", "брюки-бананы", "брюки-палаццо", "карго-шорты", "трикотажная юбка"
    }
    
    # DRESSES — платья (все типы)
    dresses = {
        "платье", "сарафан", "платье-рубашка", "shirtdress", "платье-футляр", "sheath", "а-силуэт", "а-сил", "трапеция", "платье-трапеция", "платье-комбинация", "slip dress", "платье на запах", "платье-с запахом", "wrap dress", "платье-майка", "платье-свитшот", "платье-худи", "платье-свитер", "платье-пиджак", "блейзер-дресс", "беби-долл", "babydoll", "ампир", "empire", "греческий крой", "бандажное платье", "плиссе-платье", "кружевное платье", "миди-платье", "мини-платье", "макси-платье", "вечернее платье", "коктейльное платье", "платье с баской", "пеплум-платье", "платье-лапша", "платье-рубашка деним", "слип-дресс", "платье на бретелях"
    }
    
    # OUTERWEAR — верхняя одежда (куртки, пальто, пиджаки и пр.)
    outer = {
        "куртка", "косуха", "байкерская куртка", "кожанка", "джинсовка", "джинсовая куртка", "бомбер", "блейзер", "пиджак", "жакет", "смокинг", "кардиган", "тренч", "плащ", "макинтош", "дождевик", "дощевик", "ветровка", "анорак", "парка", "пуховик", "стёганая куртка", "стеганая куртка", "пуховое пальто", "жилет утеплённый", "жилет утепленный", "безрукавка", "жилет-пуховик", "пальто", "пальто-халат", "честерфилд", "кокон", "двубортное пальто", "шинель", "дафлкот", "драп", "дубленка", "дублёнка", "авиатор", "шуба", "эко-шуба", "кейп", "накидка", "пончо", "куртка-пух", "куртка-пилот", "куртка-сафари", "кардиган-пальто", "кардиган-пончо"
    }
    
    # SHOES — обувь (максимально детально)
    shoes = {
        "обувь", "туфли", "лодочки", "pumps", "mary jane", "мэри джейн", "мери джейн", "t-bar", "т-бар", "slingback", "слингбэк", "киттен-хилз", "киттен хилз", "kitten heels", "шпильки", "stilettos", "блок-каблук", "квадратный каблук", "платформа", "танкетка", "wedge", "флэтформы", "флетформы", "броги", "оксфорды", "дерби", "монки", "лоферы", "мокасины", "балетки", "pointed flats", "peep-toe", "d'orsay", "кеды", "сникеры", "sneakers", "кроссовки", "беговые", "трейловые", "тренировочные", "теннисные", "баскетбольные", "хай-топы", "низкие кеды", "слипоны", "сабо", "кломпы", "мюли", "mule", "сандалии", "гладиаторы", "босоножки", "эспадрильи", "шлепки", "шлепанцы", "сланцы", "flip-flops", "угги", "ugg", "валенки", "дутики", "сноубутсы", "полусапоги", "сапоги", "казаки", "ковбойские сапоги", "ботинки", "ботильоны", "челси", "чукка", "desert", "дезерты", "хайкеры", "треккинговые", "гриндера", "тимберленды", "резиновые сапоги", "галоши", "резиновые", "oxfords", "brogues", "penny loafers", "tassel loafers", "ботфорты", "over-the-knee", "чулок-ботинки", "стретч-ботинки", "sock boots", "тапочки", "домашние тапочки"
    }
    
    # ACCESSORIES — аксессуары (сумки, ремни, головные уборы, украшения и пр.)
    accessories = {
        "сумка", "сумка-тоут", "тоут", "shopper", "шоппер", "багет-сумка", "багет", "baguette", "кроссбоди", "crossbody", "седло-сумка", "saddle", "хобо", "hobo", "хobo", "почтальонка", "messenger", "портфель", "рюкзак", "клатч", "minaudiere", "минодьер", "саквояж", "кейс", "чемодан", "косметичка", "кошелек", "кошелёк", "кардхолдер", "ремень", "пояс", "ремень-цепь", "корсетный пояс", "бретель для сумки", "ручка для сумки", "шарф", "платок", "палантин", "снуд", "шаль", "кашне", "галстук", "галстук-бабочка", "платок в карман", "паше", "брошь", "булавка", "заколка", "невидимка", "шпилька", "крабик", "резинка для волос", "scrunchie", "ободок", "обруч", "диадема", "тиара", "шапка", "шапка-бини", "бини", "кепка", "бейсболка", "панама", "берет", "беретка", "ушанка", "балаклава", "шляпа", "федора", "канотье", "trilby", "ковбойская шляпа", "очки", "солнечные очки", "солнцезащитные очки", "оправа", "цепочка для очков", "часы", "браслет", "браслет-манжета", "часы-браслет", "ремешок для часов", "кольцо", "перстень", "серьги", "пусеты", "кафф", "клипсы", "колье", "ожерелье", "подвеска", "кулон", "цепь", "чокер", "бусы", "бижутерия", "печатка", "запонки", "зажим для галстука", "перчатки", "варежки", "митенки", "носки", "гольфы", "гетры", "чулки", "колготки", "пояс для чулок", "ремешок-резинка", "подвязки"
    }
...
def palette_for_cvetotip(raw: Optional[str]) -> Set[str]:
    ...
    if "осен" in s:
        if "мягк" in s:
            return PALETTES_12["мягкая осень"]
        if "темн" in s:
            return PALETTES_12["тёмная осень"]
        if "тепл" in s:
            return PALETTES_12["тёплая осень"]
    
    if "весн" in s:
        if "ярк" in s:
            return PALETTES_12["яркая весна"]
        if "свет" in s:
            return PALETTES_12["светлая весна"]
        if "тепл" in s:
            return PALETTES_12["тёплая весна"]
    
    if "лет" in s:
        if "мягк" in s:
            return PALETTES_12["мягкое лето"]
        if "свет" in s:
            return PALETTES_12["светлое лето"]
        if "холод" in s:return PALETTES_12["холодное лето"]
    
    if "зим" in s:
        if "ярк" in s:
            return PALETTES_12["яркая зима"]
        if "темн" in s:
            return PALETTES_12["тёмная зима"]
...
def is_season_ok(item: Dict[str,Any], season_hint: str, temp_c: float) -> bool:
    ...
    if "зим" in season and temp_c < 7:
        return True
    if ("весн" in season or "осен" in season) and 7 <= temp_c < 23:
        return True
    if "лет" in season and temp_c >= 23:
        return True
    
    # Дополнительные эвристики по материалам
    if any(k in d for k in ["лёгк","легк","шифон","лен","льнян"]) and temp_c >= 25:
        return True
    if any(k in d for k in ["шерст","плотн"]) and temp_c < 7:
        return True
    ...
    if "лет" in season and "лет" in season_hint_norm and temp_c >= 20:  # Летние вещи только при тепле
        return True
    if "зим" in season and "зим" in season_hint_norm and temp_c < 10:  # Зимние вещи только при холоде
        return True
    if ("весн" in season or "осен" in season) and ("весн" in season_hint_norm or "осен" in season_hint_norm) and 5 <= temp_c < 25:
        return True
...
def figure_rules(figura_raw: str) -> Dict[str, Set[str]]:
    ...
    if any(k in f for k in ["перевернут","обрат","инверт"]) and "треуг" in f:
        return {
            "top_bad":{"пышные рукава","жесткие плечи","широкие плечи"},
    ...
    if any(k in f for k in ["треуг","груша"]):
        return {
            "top_good":{"объем","структур","плечики","v-образ","квадратный вырез"},
    ...
    if any(k in f for k in ["овал","яблок"]):
        return {
            "top_good":{"v-образ","легкий оверсайз","драпиров","вертикаль"},
...
def figure_pass(group: str, desc: str, rules: Dict[str,Set[str]]) -> bool:
    ...
    if group == "tops":
        if "top_bad" in rules and any(k in d for k in rules["top_bad"]):
            return False
        if "top_good" in rules and any(k in d for k in rules["top_good"]):
            return True
    
    if group == "bottoms":
        if "bottom_bad" in rules and any(k in d for k in rules["bottom_bad"]):
            return False
        if "bottom_good" in rules and any(k in d for k in rules["bottom_good"]):
            return True
    
    if group == "dresses":
        if "dress_good" in rules and any(k in d for k in rules["dress_good"]):
            return True
        if "overall_good" in rules and any(k in d for k in rules["overall_good"]):
            return True
...
def accessory_subtype(item: Dict[str,Any]) -> str:
    ...
    if tokens & {"брасл"}:
        return "bracelet"
    if tokens & {"колье", "ожерел", "кулон", "цепоч"}:
        return "necklace"
    if tokens & {"кольц"}:
        return "ring"
    if tokens & {"часы"}:
        return "watch"
    if tokens & {"платок", "шарф"}:
        return "scarf"
    if tokens & {"галстук"}:
        return "tie"
    if tokens & {"шапк", "панам", "ободок", "шляп"}:
        return "headwear"
    if tokens & {"очки"}:
        return "glasses"
...
def allowed_item(
    ...
    if not iid or iid in banned_ids:
        return False
    ...
    if not is_season_ok(item, season_hint, temp_c):
        return False
    
    if not is_style_ok(item, target_style):
        return False
    
    if group in ("tops","bottoms","dresses"):
        if not color_ok_for_palette(extract_color(item), color_palette):
            return False
        if not figure_pass(group, item.get("description",""), figure_rules(figura)):
            return False
